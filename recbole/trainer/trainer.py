...
class Trainer(AbstractTrainer):
    ...
    def _train_epoch(self, train_data, epoch_idx, loss_func=None, show_progress=False):
        ...
        for batch_idx, interaction in enumerate(iter_data):
            if self.config["model"] != "GSAU":
                interaction = interaction.to(self.device)
            else:
                graph_interaction = interaction["graph_batch"].to(self.device)
                sequential_interaction = interaction["sequential_batch"].to(self.device)
            ...
            with torch.autocast(device_type=self.device.type, enabled=self.enable_amp):
                if self.config["model"] != "GSAU":
                    ...
                    losses = loss_func(graph_interaction, sequential_interaction)
    ...
    def evaluate(
        ...
        if self.config["model"] == "GSAU":
            if self.config["predict_with_graph_encoder"]:
                eval_data = eval_data.graph_dataloader
            else:
                eval_data = eval_data.sequential_dataloader
